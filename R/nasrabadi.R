#' Fuzzy Linear Regression using Nasrabadi et al.'s Method
#'
#' The function calculates fuzzy regression coeficients using method by Nasrabadi et al. 
#' (2005) using the possibilistic approach when approximating an observed linear
...
#'    symmetric fuzzy numbers. The prediction returns symmetric triangular fuzzy
#'    number coefficients.
...
#' @examples
#' data(fuzzydat)
#' fuzzylm(y~x, fuzzydat$nas, "nas", "xl", , "yl")
...
nasrabadi <- function(x, y, omega = 0.5, sc = 1e-5){		
	...
	Z <- matrix(0, nrow = 2 * (n + 1), ncol = 2 * m)
	Ie <- diag(1, 2 * m)
	H <- omega * t(Xabs) %*% Xabs
	H <- cbind(H, Z)
	H <- rbind(H, cbind(t(Z), (1 - omega) * Ie))
	H <- 2 * H
	f <- matrix(c(-2 * omega * t(Xabs) %*% y[, 2], rep(0, 2 * m)), ncol = 1)
	Ie2 <- diag(m)
	X2 <- cbind(Xv[, 1:(n + 1)], matrix(0, ncol = n + 1, nrow = m))
	Ze <- matrix(0, m, m)
	L <- cbind(X2, -Ie2, Ze)
	L <- rbind(L, cbind(-X2, Ze, -Ie2))
	L <- rbind(L, cbind(-Xabs, Ze, Ze))
	L <- rbind(L, cbind(t(Z), -Ie))
	...
	fuzzy <- list(call = NULL, method = "nasrabadi", fuzzynum = "symmetric triangular", coef = res, 
		 		  lims = lims, x = x, y = y)
	class(fuzzy) <- "fuzzylm"
	fuzzy
}