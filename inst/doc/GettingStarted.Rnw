...
%\VignetteIndexEntry{Fitting Fuzzy Linear Regression Models in R with fuzzyreg}
%\VignettePackage{fuzzyreg}
%\VignetteDepends{FuzzyNumbers}

\title{Fitting Fuzzy Linear Regression Models in \texttt{R} with \texttt{fuzzyreg}}
...
its core, and the interval $(x_l,x_r)$t is he support of the TFN.
...
the relationship in the fuzzy set framework (Chapter 1: Quick start). For other methods,
the spreads must be imputed.
\begin{itemize}
...
When the trapezoidal fuzzy number has the core wider then one point, we need to approximate a TFN. The simplest method calculates the mean of the core as
\texttt{mean(core(B1))}.
...
observations of the response variable. \texttt{fuzzyreg} uses the min problem implementation \cite{ref:Tanaka1989} that estimates the regression coefficients in such a way that the spreads for the model
...
variable $x$ in the column \texttt{xl} and all values are equal to 0. The spreads for the
...
spreads, respectively. The arguments specifying spreads for the dependent varable are
...
the spreads correspond to the order of the variables in the \texttt{formula} argument.
...
compare the models numerically using the total error of fit $E$ \cite{ref:KimBishu1998}
...
\caption{Comparison of fuzzy regression models fitted with the MOFLR and FLS methods. The FLS
   method has lower total error of fit $E$ and thus better reflects the observations.}
\label{fig:TEF}
\end{figure}
...
the interval differs from the
confidence intervals derived from a statistical linear regression model (Figure~\ref{fig:compare}).
...
the support boundaries determined by the left and right spread (PLRLS - left panel). Statistical least-squares regression
...
\begin{quote}
\v{S}krab\'{a}nek P. and Mart\'{i}nkov\'{a} N. 2018. \emph{fuzzyreg: Fuzzy Linear Regression. R
package version 0.4}. Available from: https://CRAN.R-project.org/package=fuzzyreg.
\end{quote}
