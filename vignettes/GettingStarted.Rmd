...
```{r TNF, echo=FALSE, eval=TRUE, fig.width = 6.5, fig.height = 3.5, fig.align = 'center', fig.cap = 'Triangular fuzzy numbers.'}
...
Table: Methods for fitting fuzzy linear regression with `fuzzyreg`.
	...
	dependent variables and predictions; s -- symmetric; ns -- non-symmetric.
		...
		:--- :---: :---:  :---:  :---: :---:
		PLRLS		$\infty$		crisp		crips		nsTFN     [Lee & Tanaka1999](https://doi.org/10.15807/jorsj.42.98)
		PLR			$\infty$		crisp		sTFN		sTFN       [Tanaka et al. 1989](https://doi.org/10.1016/0377-2217(89)90431-1)
		OPLR		$\infty$		crisp		sTFN		sTFN       [Hung & Yang 2006](https://doi.org/10.1016/j.fss.2006.08.004)
		FLS			1				crisp		nsTFN		nsTFN      [Diamond 1988](https://doi.org/10.1016/0020-0255(88)90047-3)
		MOFLR	 	$\infty$	 	sTFN	 	sTFN	 	sTFN        [Nasrabadi et al. 2005](https://doi.org/10.1016/j.amc.2004.02.008)
		BFRL       1               crisp        nsTFN       nsTFN	  [Škrabánek et al. 2021](https://doi.org/10.3390/math9060685)
...
# Running a fuzzy linear regression}
...
```{r figTEF, echo=FALSE, eval=TRUE, fig.cap = 'Comparison of MOFLR and FLS model fits to the same data.', fig.width = 8.5, fig.height = 3.5, fig.align = 'center'}
oldpar <- par()
par(mfrow=c(1,2))
...
```{r regfig, eval=TRUE, echo=FALSE, fig.cap = 'Fuzzy and statistical linear regression. Fuzzy regression displays the central value and the support boundaries determined by the left and right spread (PLRLS - left panel). Statistical least-squares regressionshows the  confidence interval for the regression fit (LS - right panel).', fig.width = 8.5, fig.height = 3.5, fig.align = 'center'}
par(mfrow=c(1,2))
